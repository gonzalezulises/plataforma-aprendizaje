...
if (users.length > 0) {
  ...
  const enrollments = db.prepare('SELECT e.id, e.course_id, e.completed, c.title FROM enrollments e JOIN courses c ON e.course_id = c.id WHERE e.user_id = ?').all(userId);
  ...
  const notEnrolled = db.prepare('SELECT c.id, c.title, c.slug FROM courses c WHERE c.published = 1 AND c.id NOT IN (SELECT course_id FROM enrollments WHERE user_id = ?) LIMIT 3').all(userId);
