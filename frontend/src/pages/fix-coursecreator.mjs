...
// 1. Update the saveCourse function to include version
const oldSaveCourse = `      const response = await fetch(url, {
...
const newSaveCourse = `      // Include version for optimistic locking when updating
...
if (content.includes(oldSaveCourse)) {
  content = content.replace(oldSaveCourse, newSaveCourse);
  console.log('Updated saveCourse function');
} else {
  console.log('saveCourse pattern not found - may already be patched or different');
}

// 2. Update loadCourse to store version
const oldLoadCourse = `      setCourse(data.course);
      setCourseForm({`;

const newLoadCourse = `      setCourse(data.course);
      setCourseVersion(data.course.updated_at); // Store version for conflict detection
      setCourseForm({`;

if (content.includes(oldLoadCourse) && !content.includes('setCourseVersion(data.course.updated_at)')) {
  content = content.replace(oldLoadCourse, newLoadCourse);
  console.log('Updated loadCourse function');
}

// 3. Add conflict modal before AI Course Structure Modal
const conflictModal = `
...
if (!content.includes('showConflictModal') && content.includes('{/* AI Quiz Generator Modal */}')) {
...
console.log('CourseCreatorPage.jsx patched successfully!');
